...
const pgSql=(sql)=>{
  ...
  q=q.replace(/([A-Za-z_][A-Za-z0-9_.]*)\s*>=\s*datetime\(\s*'now'\s*,\s*(\?)\s*\)/ig,'$1::timestamp >= CURRENT_TIMESTAMP + $2::interval');
  ...
  q=q.replace(/strftime\(\s*'%Y-%m'\s*,\s*([^)]+)\)/ig,(_,expr)=>`to_char(${expr}::timestamp,'YYYY-MM')`);
  q=q.replace(/strftime\(\s*'%Y-%W'\s*,\s*([^)]+)\)/ig,(_,expr)=>`to_char(${expr}::timestamp,'IYYY-IW')`);
