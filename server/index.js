...
try{db.exec("CREATE TABLE IF NOT EXISTS app_settings(key TEXT PRIMARY KEY,value TEXT NOT NULL)");db.prepare("INSERT OR IGNORE INTO app_settings(key,value) VALUES('currency','EUR'),('odds_format','dot'),('green_color','#16a34a'),('red_color','#dc2626'),('accent_color','#2563eb'),('theme','light'),('stake_limit_percent','2')").run()}catch(e){console.error(e)}
...
app.patch('/api/admin/settings',requireAdmin,(req,res)=>{const allowedCurrency=['EUR','USD','GBP','BRL','CHF','CAD','AUD','JPY','PLN','SEK','NOK','DKK'];const allowedOdds=['dot','comma'];const allowedTheme=['light','dark','system'];if(req.body.currency&&!allowedCurrency.includes(req.body.currency))return res.status(400).json({error:'Moeda não suportada.'});if(req.body.odds_format&&!allowedOdds.includes(req.body.odds_format))return res.status(400).json({error:'Formato de odds inválido.'});if(req.body.theme&&!allowedTheme.includes(req.body.theme))return res.status(400).json({error:'Tema inválido.'});if(req.body.stake_limit_percent!=null){const v=Number(req.body.stake_limit_percent);if(!Number.isFinite(v)||v<0||v>100)return res.status(400).json({error:'A percentagem de stake deve estar entre 0 e 100.'});}for(const k of ['green_color','red_color','accent_color'])if(req.body[k]&&!/^#[0-9a-fA-F]{6}$/.test(String(req.body[k])))return res.status(400).json({error:'Cor inválida.'});for(const k of ['currency','odds_format','green_color','red_color','accent_color','theme','stake_limit_percent'])if(req.body[k]!=null&&req.body[k]!=='')run('INSERT INTO app_settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value',k,req.body[k]);res.json({ok:true})});
...
app.get('/api/countries',(req,res)=>res.json(all('SELECT * FROM countries WHERE user_id IS NULL OR user_id=? ORDER BY name',req.user.id)));
