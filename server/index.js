...
// Garantir explicitamente o Sporting CP no catálogo global, mesmo quando a fonte externa não o devolve.
db.prepare("INSERT INTO teams(user_id,name,country,team_type,source) SELECT NULL,'Sporting CP','Portugal','club','seed' WHERE NOT EXISTS (SELECT 1 FROM teams WHERE user_id IS NULL AND lower(trim(name))=lower(trim('Sporting CP')))").run();
...
app.get('/api/countries',(req,res)=>res.json(all('SELECT * FROM countries ORDER BY name')));app.get('/api/teams',(req,res)=>{const q=String(req.query.search||'').trim().toLowerCase(),type=String(req.query.type||'').trim(),country=String(req.query.country||'').trim(),like='%'+q+'%';const where=['(user_id IS NULL OR user_id=?)'],args=[req.user.id];if(q){where.push('(lower(name) LIKE ? OR lower(COALESCE(country,\'\')) LIKE ? OR EXISTS (SELECT 1 FROM team_translations ttq WHERE ttq.team_id=teams.id AND lower(ttq.name) LIKE ?))');args.push(like,like,like)}if(type){where.push('team_type=?');args.push(type)}if(country){where.push('lower(COALESCE(country,\'\'))=lower(?)');args.push(country)}const paginate=String(req.query.paginate||'')==='1';const requestedPage=Math.max(Number(req.query.page)||1,1);const perPage=Math.min(Math.max(Number(req.query.perPage)||25,1),100);const total=Number(all('SELECT COUNT(*) total FROM teams WHERE '+where.join(' AND '),...args)[0]?.total||0);const totalPages=Math.max(1,Math.ceil(total/perPage));const page=Math.min(requestedPage,totalPages);const offset=(page-1)*perPage;const limit=paginate?perPage:Math.min(Math.max(Number(req.query.limit)||1000,1),1000);const rows=all('SELECT * FROM teams WHERE '+where.join(' AND ')+' ORDER BY lower(name),id LIMIT '+limit+(paginate?' OFFSET '+offset:''),...args);for(const row of rows){row.translations={};for(const t of all('SELECT language,name FROM team_translations WHERE team_id=?',row.id))row.translations[t.language]=t.name}if(paginate)return res.json({items:rows,total,page,perPage,totalPages});res.json(rows)});
...
const teamCanonicalAliases={
  ...
  'Sporting CP':['Sporting CP','Sporting','Sporting Clube de Portugal','Sporting Lisbon','Sporting CP SAD','Sporting Clube de Portugal - Futebol, SAD'],
...
app.delete('/api/houses/:id',(req,res)=>{const id=req.params.id;try{db.transaction(()=>{run('UPDATE bets SET house_id=NULL WHERE house_id=? AND user_id=?',id,req.user.id);run('UPDATE transactions SET house_id=NULL WHERE house_id=? AND user_id=?',id,req.user.id);run('UPDATE bonuses SET house_id=NULL WHERE house_id=? AND user_id=?',id,req.user.id);run('DELETE FROM houses WHERE id=? AND user_id=?',id,req.user.id)})();res.json({ok:true})}catch(e){res.status(400).json({error:e.message})}});
