...
db.exec(`CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,email TEXT NOT NULL UNIQUE,password_hash TEXT NOT NULL,role TEXT NOT NULL DEFAULT 'user',active INTEGER NOT NULL DEFAULT 1,currency TEXT NOT NULL DEFAULT 'EUR',created_at TEXT DEFAULT CURRENT_TIMESTAMP);
...
for(const q of ["ALTER TABLE users ADD COLUMN role TEXT NOT NULL DEFAULT 'user'","ALTER TABLE users ADD COLUMN active INTEGER NOT NULL DEFAULT 1","ALTER TABLE houses ADD COLUMN logo TEXT","ALTER TABLE users ADD COLUMN currency TEXT NOT NULL DEFAULT 'EUR'","ALTER TABLE bets ADD COLUMN bet_type TEXT DEFAULT 'single'","ALTER TABLE bets ADD COLUMN selections TEXT","ALTER TABLE bets ADD COLUMN combined_odds REAL"]){try{db.exec(q)}catch(e){}}
...
app.patch('/api/auth/currency',(req,res)=>{const u=currentUser(req),allowed=['EUR','USD','GBP','BRL','CHF','CAD','AUD','JPY','PLN','SEK','NOK','DKK'];if(!u)return res.status(401).json({error:'Não autenticado'});const currency=String(req.body.currency||'EUR').toUpperCase();if(!allowed.includes(currency))return res.status(400).json({error:'Moeda não suportada.'});db.prepare('UPDATE users SET currency=? WHERE id=?').run(currency,u.id);res.json({ok:true,currency})});app.post('/api/auth/logout',(req,res)=>{const t=cookies(req).bt_session;if(t)db.prepare('DELETE FROM sessions WHERE token=?').run(t);setCookie(res,'bt_session','',0);res.json({ok:true})});
app.get('/api/auth/me',(req,res)=>{const u=currentUser(req);if(!u)return res.status(401).json({error:'Não autenticado'});res.json({user:{id:u.id,name:u.name,email:u.email,role:u.role,currency:u.currency||'EUR'}})});
