...
CREATE TABLE IF NOT EXISTS houses(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,name TEXT NOT NULL,username TEXT,logo TEXT,bonus_day TEXT,bonus_conditions TEXT,bonus_weekly INTEGER NOT NULL DEFAULT 0,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id));
...
try{db.exec("CREATE TABLE IF NOT EXISTS app_settings(key TEXT PRIMARY KEY,value TEXT NOT NULL)");db.prepare("INSERT OR IGNORE INTO app_settings(key,value) VALUES('currency','EUR'),('odds_format','dot'),('green_color','#16a34a'),('red_color','#dc2626'),('accent_color','#2563eb'),('theme','light')").run()}catch(e){console.error(e)}
...
app.get('/api/admin/competition-catalog',requireAdmin,(req,res)=>{const country=db.prepare('SELECT name FROM countries WHERE id=?').get(req.query.country_id);res.json(country&&Array.isArray(catalog[country.name])?catalog[country.name]:[])});app.get('/api/admin/competition-catalog-all',requireAdmin,(req,res)=>{const rows=[];for(const country of all('SELECT id,name FROM countries ORDER BY name'))for(const name of (catalog[country.name]||[]))rows.push({country_id:country.id,country_name:country.name,name});res.json(rows)});
app.get('/api/admin/competitions',requireAdmin,(req,res)=>res.json(all('SELECT c.*,p.name country_name,p.logo country_logo FROM competitions c JOIN countries p ON p.id=c.country_id ORDER BY p.name,c.name').sort(competitionOrder)));app.get('/api/admin/competition-validation',requireAdmin,(req,res)=>{const invalid=all('SELECT c.id,c.name,c.country_id,p.name country_name FROM competitions c JOIN countries p ON p.id=c.country_id ORDER BY p.name,c.name').filter(x=>Array.isArray(catalog[x.country_name])&&!catalog[x.country_name].includes(x.name));res.json({invalid,count:invalid.length})});
app.post('/api/admin/competitions',requireAdmin,(req,res)=>{try{const country=db.prepare('SELECT name FROM countries WHERE id=?').get(req.body.country_id);const name=String(req.body.name||'').trim();if(!country||!name)return res.status(400).json({error:'Seleciona o país e indica a competição.'});if(Array.isArray(catalog[country.name])&&!catalog[country.name].includes(name))return res.status(400).json({error:'Esta competição não pertence ao país selecionado segundo o catálogo validado.'});run('INSERT INTO competitions(country_id,name,logo) VALUES(?,?,?)',req.body.country_id,name,req.body.logo||'');res.json({ok:true})}catch(e){res.status(400).json({error:'Competição já existe neste país.'})}});
app.put('/api/admin/competitions/:id',requireAdmin,(req,res)=>{const country=db.prepare('SELECT name FROM countries WHERE id=?').get(req.body.country_id);const name=String(req.body.name||'').trim();if(!country||!name)return res.status(400).json({error:'Seleciona o país e indica a competição.'});if(Array.isArray(catalog[country.name])&&!catalog[country.name].includes(name))return res.status(400).json({error:'Esta competição não pertence ao país selecionado segundo o catálogo validado.'});run('UPDATE competitions SET country_id=?,name=?,logo=? WHERE id=?',req.body.country_id,name,req.body.logo||'',req.params.id);res.json({ok:true})});
...
app.get('/api/competitions',(req,res)=>res.json(all('SELECT c.*,p.name country_name,p.logo country_logo FROM competitions c JOIN countries p ON p.id=c.country_id ORDER BY p.name,c.name').sort(competitionOrder)));
...
app.get('/api/houses',(req,res)=>res.json(all('SELECT id,name,username,logo,bonus_day,bonus_conditions,bonus_weekly,created_at FROM houses WHERE user_id=? ORDER BY name',req.user.id)));
app.post('/api/houses',(req,res)=>{const x=req.body;run('INSERT INTO houses(user_id,name,username,logo,bonus_day,bonus_conditions,bonus_weekly) VALUES(?,?,?,?,?,?,?)',req.user.id,x.name,x.username||'',x.logo||'',x.bonus_weekly?x.bonus_day||'':'',x.bonus_conditions||'',x.bonus_weekly?1:0);res.json({ok:true})});
app.put('/api/houses/:id',(req,res)=>{const x=req.body;run('UPDATE houses SET name=?,username=?,logo=?,bonus_day=?,bonus_conditions=?,bonus_weekly=? WHERE id=? AND user_id=?',x.name,x.username,x.logo||'',x.bonus_weekly?x.bonus_day||'':'',x.bonus_conditions||'',x.bonus_weekly?1:0,req.params.id,req.user.id);res.json({ok:true})});
